---
title: "Understanding IOptions, IOptionsMonitor, and IOptionsSnapshot in C#"
date: 2023-05-17T09:19:16+02:00
...
Or you can get the best out of **Strongly Typed Configurations** - you won't have to care about manual casting, because everything is already done for you by .NET.
...
As you can see, it take the content with name "MyConfig" and maps it to an object of type `GeneralConfig`.

To test such types, I've created a set of dummy API Controllers. These Controllers expose one single method, `Get()`, that reads the value from configuration and returns it in an object.

We are going to inject `IOptions`, `IOptionsSnapshot`, and `IOptionsMonitor` within the `TestingController` constructor of the related Controllers, so that we can try the different approaches.

## IOptions&lt;T&gt;: Simple, Singleton

`IOptions<T>` is the most simple way to inject such configurations. We can inject it in our constructors and access the actual value using the `Value` property:
...
* this service is injected as a **Singleton instance**: the whole application uses the same instance, and it is valid throught the whole application lifetime.
...
**It works, but it's useless**: since `IOptions<T>` is a Singleton, we don't need to access the `Value` property every time. It won't change during time, and accessing it every single time is a useless operation. We can use the former approach: it's easier to write and (just a bit) more performant.
...
There is no way to test a `IOptionsSnapshot<T>` as we did with `IOptions<T>`, so you have to use stubs or mocks (maybe with [Moq or NSubstitute 🔗](https://code4it.dev/blog/moq-vs-nsubstitute-syntax/)).
...
![](./ioptionssnapshot.gif)

I performed the following steps:

1. run the application
2. call the */TestingIOptionsSnapshot* endpoint. Ther returned value is the same on the appsettings.json file: Davide Bellone.
...
1. the name of the property that stores the config value is not `Value` but `CurrentValue`.
...
Again, there is no way to test a `IOptionsMonitor<T>` as we did with `IOptions<T>`. So you should rely on stubs and mocks (again, maybe with [Moq or NSubstitute 🔗](https://code4it.dev/blog/moq-vs-nsubstitute-syntax/)).

### Demo

In the GIF below I demonstrate the usage of `IOptionsMonitor`.

I created an API controller that listens to changes on the configuration, updates a static counter, and returns the final result from the API:
...
3. I then update and save the config file. In background, the `OnChange` callback is fired, and the Updates value is updated;

Oddly, the callback is called more times than expected. I updated the file only twice, but the counter is set to 6. That's a weird behavior. If you know why it happens, drop a message below 📩

## IOptions&lt;T&gt; vs IOptionsSnapshot&lt;T&gt; vs IOptionsMonitor&lt;T&gt; in .NET

We've seen a short introduction to IOptions, IOptionsSnapshot, and IOptionsMonitor.

There are some differences, of course. Here's a table with a recap of what we learned with this article.
...
https://code4it.dev/blog/how-to-set-configurations-values-dotnet/

https://code4it.dev/blog/dependency-injection-lifetimes/

come testare IOptionssnapsiot e monitor